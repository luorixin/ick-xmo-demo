 	...
 	BS.Ncity = function(option) {
        ...
        if(self.lang==="en"){
           // $(".cityCon",self.container).css("white-space","nowrap");
            ...
            $(".cityCon",self.container).css("width","111px");
            $(".township",self.container).css("left","121px");
        }
        ...
        $(self.container).on("click",".countryName",function(){
            ...
            if("foregin"===type){
                ...
                $(".citySelectArea",self.container).css("height","277");
            }
        });
    ...
    /* 正则表达式 筛选中文城市名、拼音、首字母 */
 	...
 	BS.Ncity.prototype = {
        ...
        createUl:function(d){
            ...
            if($(".x-body").length>0){
               inputPos.left =inputPos.left - $(".x-body").offset().left-150;
            }
            $(".citySelector").remove();
            ...
            div.style.top = $(d).outerHeight() + 'px';
        ...
        convertChina:function(d,type,thi){
            ...
            var result=[],levels=[],
                ...
                obj={},objprovince={},city=[],i,j,k,l,m,n,temp=[],objResult,name, areaFY;
            var area = self.getAreaArr(d); 
            type=type==="en"?"":"_cn";
        ...
        getAreaArr:function(d){
            var self = this;
            var area=[];
            for ( i = d.length - 1; i >= 0; i--) {
                if(area.length>0){
                    if(d[i].area && self.check(area,d[i].area)<0){
                        area.push(d[i].area);
                    }
                }else{
                    if (d[i].area) {
