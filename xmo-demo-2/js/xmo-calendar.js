function xmoCalendar(options){
  ...
  _this.arrowLeft = options['arrowLeft'] || 0;
...
xmoCalendar.prototype.init_html = function(){
  ...
  var baseHtml = '<div class="xmoCalendarWraper'+ excludeClass +' xmoCalendar'+ _this.type +'" id="'+ _this.boxId +'"><svg class="svg-triangle" style="margin-left:'+_this.arrowLeft+/*+($(_this.inputId).offset().left)+*/'px" version="1.1" xmlns="http://www.w3.org/2000/svg"><polygon style="fill:#fff; stroke:#EEEEEE;stroke-width:1" points="0,9 6,0 12,9 "/><polygon style="fill:#fff; stroke:#fff;stroke-width:1" points="0,9 12,9 "/></svg><div class="xmoCalendarInner"><div class="xmoCalendarDataRange"><div style="height:40px;text-align:left;"><label style="line-height:40px;text-indent:1em;">'+lang['startFrom']+'</label></div><div style="min-height:228px;border-top:1px solid #dfdfdf;"><dl>'+ menuHtml +'</dl></div></div><div class="xmoCalendarMainArea"><div class="row xmoCalendarMainHead"><div class="span" style="margin-left:5px;margin-right:10px;"><label class="sub_txt2"><input type="text" class="xmoCalendarFrom" placeholder="YYYY-MM-DD"></label></div><div class="span"><label class="sub_txt2"><span style="margin-right:10px;">'+lang['to']+'</span><input type="text" placeholder="YYYY-MM-DD" class="xmoCalendarTo"></label></div><div class="xmoCalendarError"></div></div><div class="cle"></div><div class="xmoCalendarTitle"><table><tr><td class="xmoCalendarMonth"></td></tr></table><div class="xmoCalendarPrev"></div><div class="xmoCalendarNext"></div></div><div class="xmoCalendarList"><div class="xmoCalendarTableWraper xmoCalendarTableWraperLast"></div><div class="cle"></div></div><div class="xmoCalendarClear"></div></div><div class="cle"></div><div class="clearfix xmoCalendarTableTipsDiv"><div class="pull-left t_padding_10 xmoCalendarTableTips"></div>'+ CompositeAllHtml +'<div class="outer_submit"><a href="javascript:;" class="cancel">'+ lang['cancel'] +'</a>&nbsp;&nbsp;<a href="javascript:;" class="submit">'+ lang['submit'] +'</a></div></div></div></div>';
  $('body').append(baseHtml);
  if (_this.menu.length==0) {
    $('.xmoCalendarDataRange',_this.wrapperBox).hide()
  }else{
    $('.xmoCalendarList',_this.wrapperBox).attr("style","border-left:1px solid #dfdfdf;");
  }
...
xmoCalendar.prototype.closeBox = function(){
    this.inputObj.parent().removeClass("xmoCalendar_focus");
...
xmoCalendar.prototype.init_Event = function(){
  ...
  $(document).on('click','#' + _this.boxId + ' .cancel',function(e){
    _this.closeBox();
  })
...
xmoCalendar.prototype.getScrollTop= function()
{
    ...
    for (var i = 0; i < 6; i++) {
      ...
      if(weekAry == undefined){
        ...
        for (var j = 0; j < weekAry.length; j++) {
          var item = weekAry[j];
          var aClass = '';
          if (item.month!=_this.currentFirstMonth) {
            tr += '<td><span >'+item.day+'</span></td>';
            ...
            if(item.day == 0){
              ...
              if(_this.selectedDone){
                var isInRange = _this.isInRange(thisDay);
                if(isInRange){
                  aClass = 'selectedInRange selected';
                }
...
xmoCalendar.prototype.getMonthData = function(year,month){
    ...
    var intDaysInLastMonth = _this.getDaysInMonth(lastMonth-1,lastyear);

    var lastdaily=0;
    var prevdaily=intDaysInLastMonth-startDay;
     // 共有几周
      ...
      for (var intDay = 0; intDay < 7; intDay++) {
        ...
        if ((daily > 0) && (daily <= intDaysInMonth)) {
          ...
          if (daily>intDaysInMonth) {
            dayObj.day = ++lastdaily;
            dayObj.month = month+1;
          }else{
            dayObj.day = ++prevdaily;
            dayObj.month = month-1;
          }
        }
...
xmoCalendar.prototype.lang = {
  ...
  'EN' : {
      ...
      yes : 'Yes',
      allWeekStr : ['You have excluded','Exclude ','th in every month?'],
      allWeekStr1 : [' is excluded.','Exclude ',' in every month?'],
      allWeekStrUnselect : ['You have selected','Select ','th in every month?'],
